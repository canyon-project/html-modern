...
import { resolveReportHtmlDist, writeHtmlReport, writeReportDataJson } from "./write-report.js";

const require = createRequire(import.meta.url);
const { name: packageName, version: packageVersion } = require(
  require.resolve("@canyonjs/html-modern/package.json"),
) as {
  name: string;
  version: string;
};
...
export class CoverageReport {
  ...
  async generate({
    ...
    if (reportHtmlDist) {
      ...
      console.log("@canyonjs/html-modern dist/page not found, skipping HTML report generation");
    }
