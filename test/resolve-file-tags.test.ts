...
import { collectAvailableTags, resolveFileTags } from "../src/reporter/resolve-file-tags";
...
describe("resolveFileTags", () => {
  it("assigns multiple tags to one file", () => {
    const tags = resolveFileTags(["src/payments/api.ts"], [
  ...
  it("omits files with no matching tags", () => {
    const tags = resolveFileTags(["src/auth/login.ts"], [{ glob: "src/payments/**", tag: "payments" }]);
    assert.deepEqual(tags, {});
  });

  it("normalizes backslashes before matching", () => {
    const tags = resolveFileTags(["src\\payments\\api.ts"], [{ glob: "src/payments/**", tag: "payments" }]);
